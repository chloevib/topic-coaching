...
> 交接方式：本任务报告里的「交办每日任务」小节，写成每日任务能直接执行的条目（哪个文件、补哪个字段、目标关键词）。**不要**直接去改 `config/categories/*.ts` 的文案——工作区可能有每日任务未提交的改动，会打架。
...
```
# topic-coaching 每周设计 + SEO 维护报告 · <日期>
...
## 3. 前端技术 SEO
- 已修：<标题层级 / schema / metadata / 内链 / CWV → 文件>
- 风险项：<列表>

## 4. 内容 SEO / GEO 审计 → 交办每日任务
- GEO 字段覆盖率：<x%>（上周 <y%>）
- 待办：<文件> | <缺什么> | <目标关键词> | <优先级>

...
>
> **与每周任务的分工**：本任务管**内容**——`config/site.ts` 与 `config/categories/*.ts` 的文案字段、quiz 收录、draft 转正、选题调研。站点**外观与前端技术 SEO**（`src/**`：组件、页面、globals.css、lib/seo.ts、sitemap/robots/llms.txt、结构化数据、CWV）归 `docs/WEEKLY_DESIGN_SEO_MAINTENANCE.md`，本任务不碰。每周任务审计出的内容缺口会写成「交办每日任务」清单，可直接拿来执行。
>
